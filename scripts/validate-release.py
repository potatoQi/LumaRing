...
def validate(assets, root=ROOT, require_distribution=False):
    ...
    if require_distribution:
        with tempfile.TemporaryDirectory(prefix='lumaring-release-verify-') as folder:
            ...
            subprocess.run(['xcrun', 'stapler', 'validate', extracted], check=True)
            subprocess.run(['spctl', '--assess', '--type', 'execute', extracted], check=True)
...
if __name__ == '__main__':
    ...
    try:
        print(json.dumps(validate(args.assets, require_distribution=args.require_distribution), indent=2))
