...
`WindowService.close` presses the exact window’s enabled AX close button on its existing serial worker. It checks the window and button process IDs and roles, with bounded IPC timeouts; it never sends Command-W or falls back to closing another window. `BrowserEvents.close` resolves the tab’s latest window, then addresses both objects by unique text ID for the `core/clos` command. Reordering between query and close cannot redirect the command to a neighbour. `RingController` dismisses the ring before these requests; the next invocation queries fresh state. Closing never activates, raises, reopens or unminimizes the target. Tab closing does not change the active tab or window index. Normal quit also avoids activating the target. Save/confirmation dialogs remain under the target application’s control.
