...
`WindowNamesTests` verifies natural name sorting and deterministic ties, alias persistence and reset, changed tab titles/positions, duplicate-target isolation, process restart protection, limited-result retention and complete-result pruning. It checks that preview matching still uses original titles, preview headings display aliases, secondary menus capture the correct sorted target, and renaming cancels pressed targets while preserving the renamed item's visible page. Paging tests use numbered names to make their expected ordering independent of the host's language.

For manual validation, right-click disposable windows and tabs, rename several with duplicate and numeric names, cancel a rename, clear an alias and reopen the ring. Confirm names survive LumaRing relaunch while the target app stays running, and are reset for new target-app processes. Test title changes, tab movement, background close and both display modes. Open the Edit floating panel, release Option/the invocation shortcut, click outside, and dismiss/reopen the ring; no stale editor callback should rename a different target or trigger a switch.
